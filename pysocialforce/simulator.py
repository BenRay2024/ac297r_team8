...
"""
import numpy as np
from pysocialforce.utils import DefaultConfig, CustomConfig
...
class Simulator:
    ...
    def __init__(self, state, groups=None, obstacles=None, config_file=None):
        ...
        if config_file:
            self.config = CustomConfig(config_file)
    ...
    def step(self, n=1, new_pedestrians=None):
        """Step n times and introduce new pedestrians if specified."""
        for i in range(n):
            # Check if there are new pedestrians to add at this timestep
            if new_pedestrians and i in new_pedestrians:
                # Add all new pedestrians for this timestep in one batch
                new_peds_batch = np.array(new_pedestrians[i])
                self.peds.add_pedestrians(new_peds_batch)
