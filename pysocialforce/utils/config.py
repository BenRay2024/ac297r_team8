...
class CustomConfig(Config):
    def __init__(self, config_dict=None):
        if config_dict:
            # If a config dictionary is provided, convert it to a TOML string and load it
            config_toml = toml.dumps(config_dict)
            super().__init__(toml.loads(config_toml))
        else:
            # Otherwise, load the default configuration
            super().__init__(toml.loads(DefaultConfig.CONFIG))