...
class SceneVisualizer:
    ...
    def __init__(
        self, scene, output=None, writer="imagemagick", cmap="viridis", agent_colors=None, density=False, **kwargs
    ):
        self.scene = scene
    ...
    def plot(self):
        ...
        if not groups:
            for ped in range(self.scene.peds.size()):
                ...
                # Create a mask where both x and y are not zero
                mask = (x != 0) | (y != 0)
                ...
                self.ax.plot(x_filtered, y_filtered, "-o", label=f"ped {ped}", markersize=2.5)
        else:
    ...
    def plot_density(self, timestep):
        # Extract pedestrian positions for the given timestep
        positions = self.states[timestep, :, :2]  # Assuming positions are the first two columns
        valid_positions = positions[~np.all(positions == 0, axis=1)]  # Filter out (0,0) positions

        # Generate a KDE plot of the positions
        if len(valid_positions) > 0:  # Check if there are any valid positions to plot
            sns.kdeplot(
                x=valid_positions[:, 0],
                y=valid_positions[:, 1],
                cmap="Reds",
                fill=True,
                bw_adjust=0.5,
                ax=self.ax  # Plot on the existing axis
            )

    def animate(self, density=False):
    ...
    def __enter__(self):
        ...
        self.ax.set(xlim=(5, 25), ylim=(15, 35))
    ...
    def animation_update(self, i):
        if self.density:
            self.ax.clear()  # Clear the axis to redraw the heatmap
            ...
            self.ax.grid(linestyle="dotted")
            self.ax.set_aspect('equal')
            self.plot_obstacles()  # Redraw the obstacles
            
            self.plot_density(i)  # Update the heatmap for the current timestep
